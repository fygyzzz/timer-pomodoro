...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function validateTimeInput(e) {
    ...
    if (input.id === "pomodoro-time") {
      maxValue = 90;
    } else if (input.id === "short-break-time") {
      maxValue = 30;
    } else if (input.id === "long-break-time") {
      maxValue = 90;
    }

    // Если значение больше максимального или не число
    if (input.value > maxValue || isNaN(input.value)) {
      showNotification(`Максимальное значение: ${maxValue} минут`, "error");
      input.value = maxValue;
    }

    // Если значение меньше 1
    if (input.value < 1) {
      showNotification("Минимальное значение: 1 минута", "error");
      input.value = 1;
    }
  }
  ...
  function allowOnlyNumbers(e) {
    // Разрешаем только цифры, Backspace, Delete, Tab и стрелки
  ...
  pomodoroTimeInput.addEventListener("input", validateTimeInput);
  shortBreakTimeInput.addEventListener("input", validateTimeInput);
  longBreakTimeInput.addEventListener("input", validateTimeInput);

  pomodoroTimeInput.addEventListener("keydown", allowOnlyNumbers);
  shortBreakTimeInput.addEventListener("keydown", allowOnlyNumbers);
  longBreakTimeInput.addEventListener("keydown", allowOnlyNumbers);
  ...
  function startTimer() {
    ...
    if (currentMode === "pomodoro") {
      currentTime = Math.min(parseInt(pomodoroTimeInput.value) || 25, 90);
    } else if (currentMode === "shortBreak") {
      currentTime = Math.min(parseInt(shortBreakTimeInput.value) || 5, 30);
    } else {
      currentTime = Math.min(parseInt(longBreakTimeInput.value) || 15, 90);
    }
  ...
  function renderTasks() {
    ...
    tasks.forEach((task) => {
      ...
      taskItem.className = `task-item ${
        task.id === selectedTaskId ? "selected-task" : ""
      }`;
      taskItem.innerHTML = `
                <span>${task.text}</span>
                <div>
  ...
  async function saveSettings() {
    const settings = {
      pomodoroTime: Math.min(
        Math.max(1, parseInt(pomodoroTimeInput.value) || 25),
        90
      ),
      shortBreakTime: Math.min(
        Math.max(1, parseInt(shortBreakTimeInput.value) || 5),
        30
      ),
      longBreakTime: Math.min(
        Math.max(1, parseInt(longBreakTimeInput.value) || 15),
        90
      ),
